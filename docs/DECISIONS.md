...
| **Jobcu does not read Adzuna's website pages**, only its official API. Adzuna jobs are scored from the API's short summary (title, company, first ~500 characters) unless the same job is also found with a full ad elsewhere. Cards say "Scored from a short summary of the ad". **Waiting for the owner's decision.** | Adzuna's website blocked automated access to its own terms page, so its rules for automated reading couldn't be checked. The owner asked for no legal risk. |
