...
| **Google Maps is the preferred source for public-transport times** (owner's suggestion): trips during normal working hours, e.g. departing between 07:00 and 19:00 on a weekday. It will be an **optional key**: without it, Jobcu shows a clearly labelled estimate. The final decision comes in Phase 2, after checking Google's current price, free allowance and terms, and the owner approves before any billing is set up. | Google has the most complete public-transport timetables across Germany, Ireland and the UK. It's a new outside service with a payment card on file, so the owner's approval and the terms check come first. Keeping it optional keeps setup simple for friends. |
