...
| **His limits for now: €20 a month for the AI, and Google Maps only within its free allowance** (€1 at most). The AI's limit is Google's spend cap on the budget "Jobcu AI", which pauses the Gemini API at €20. Maps can't have a spend cap, so Google's own daily quota stops it instead: **300 route-matrix elements a day** (at most about 9,300 a month, below the free 10,000), **0 ComputeRoutes requests a day** (Jobcu never uses them), plus a €1 email alert on the budget "Jobcu Google Maps". Jobcu's own monthly stop (9,000) stays as well; when a stop is reached, travel times fall back to AI estimates. | The owner: "set limits for 20€ for API and 1€ maps, I don't want to spend more for now". This also answers the 2026-09-17 request for a Google-side daily limit. |
...
| **Google's default services stay switched on** in the Maps project (BigQuery, Cloud Storage and the like). | They cost nothing, the key can only reach the Routes API, and switching some of them off can break the console's own pages. |
