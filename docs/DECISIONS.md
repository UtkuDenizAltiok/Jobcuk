...
| **Open question for the owner:** may researched facts that barely change (a town's population, the last election's results) be remembered with their source and date for a while, or must every search research them again? | Re-researching everything makes searches slower and uses more AI. The owner's rule so far is "everything about the job search stays fresh"; this is about background facts, not about jobs. |
...
| **Where the job is:** the coordinates a job site gives, otherwise the centre of the job's town. With a Google Maps key, jobs within 10 minutes of the limit are measured again from the company's own address (Places text search). | The owner asked for the exact company location when possible, otherwise the best estimate. Looking up every company would use the free allowance quickly; only close calls need it. |
